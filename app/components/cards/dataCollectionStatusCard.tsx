...
export default function DataCollectionStatus() {
    ...
    return (
    <div className="w-full lg:w-[50%] relative shadow-sm border border-gray-100 p-4 rounded-xl bg-white">
      <div className="flex gap-2 items-center mb-3">
        <p className="font-bold text-sm text-black">Data Collection Status</p>
        <Image src="/info.png" alt="info" width={15} height={15}/>
      </div>

      <div className="grid grid-cols-[1.2fr_0.8fr_0.5fr_0.5fr] text-[0.8rem] border-b border-gray-200 py-1.5 font-semibold text-gray-900">
        <span>Site</span>
      ...
      <div className="pb-3">
        {dataCollection.map((status) => (
          <div
            key={status.id}
            className="grid grid-cols-[1.2fr_0.8fr_0.5fr_0.5fr] py-1.5 border-b border-gray-200 text-[0.8rem] text-gray-900"
          >
      ...
      <div className="flex">
        <Link className="text-sm hover:text-blue-700 absolute bottom-2 right-4 font-semibold ml-auto text-blue-500 mt-4" href="#">View All Sites <span>&rarr;</span></Link>
      </div>
    </div>
  );
}
